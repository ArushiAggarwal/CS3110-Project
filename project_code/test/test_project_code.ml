...
(* let test_random = "Test suite to ensure pseudorandom generator works with\n
   respect to seed." >::: [ ( "Ensure seed 42 works" >:: fun _ -> assert_equal [
   5; 6; 3; 4; 7 ] (generate_guess 42) ); ] *)
...
(* let test_knuth = "Test Knuth algorithm returns correctly." >::: [ ("Test
   correct base case answer" >:: let () = print_endline "hi" in let solution, _
   = knuth_algorithm [ 1; 2; 3; 4 ] in fun _ -> assert_equal [ 1; 2; 3; 4 ]
   solution); ("Test other answer" >:: let () = print_endline "hi" in let
   solution2, _ = knuth_algorithm [ 6; 1; 5; 2 ] in fun _ -> assert_equal [ 6;
   1; 5; 2 ] solution2); ] *)

let suite = "Final project test suite" >::: [ test_pin ]
